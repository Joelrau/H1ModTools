...
hastag( model, tag )
{
    ...
    {
        if ( tolower( getpartname( model, partIndex ) ) == tolower( tag ) )
            return 1;
    }
...
destructible_state( tagName, modelName, health, validAttackers, validDamageZone, validDamageCause, grenadeImpactDeath, splashRotation )
{
	...
	// if tag is not found we return
	if( isDefined( tagName ) && !hastag( level.destructible_type[ destructibleIndex ].parts[ 0 ][ 0 ].v[ "modelName" ], tagName ) )
        return;
